...
import { MaintenancePriorityService } from '../../../_services/maintenance-priority.service';
import { MaintenancePrioritySharedService } from '../../../_services/maintenance-priority-shared.service';

declare function setFootable(tableName: string): any;
declare function footableIni(tableName: string): any;

@Component({
  selector: 'app-maintenance-priorities-list',
  templateUrl: './maintenance-priorities-list.component.html',
  styleUrls: ['./maintenance-priorities-list.component.scss']
})
export class MaintenancePrioritiesListComponent implements OnInit, AfterViewInit {

  objName = 'Prioridad de mantenimiento';
  ids: number[];
  objs: MaintenancePriority[];
  objsFiltered: MaintenancePriority[];
  currentDate: string;
  isLoadingData = false;
  entityType = EntityType.MaintenancePriority;
  ...
  constructor(
    ...
    private objSharedService: MaintenancePrioritySharedService,
  ...
  onSuccess(obj: MaintenancePriority[]) {
    this.objs = obj;
    if (this.objs) {
  ...
  getOptionButtonBar(row: MaintenancePriority): OptionButtonBar {
